...
#joy{position:absolute;left:18px;bottom:20px;width:148px;height:148px;border:2px solid #9dc8e766;border-radius:50%;background:radial-gradient(circle,#ffffff12,#ffffff04);pointer-events:auto;display:none;z-index:40;touch-action:none}
#knob{position:absolute;left:48px;top:48px;width:52px;height:52px;border-radius:50%;background:#b8dcff66;border:1px solid #d7edffbb;box-shadow:0 0 18px #5dbdff33}
#aimZone{position:absolute;right:0;top:18%;width:58%;height:58%;z-index:25;display:none;touch-action:none;pointer-events:auto}
.mobileBtns{position:absolute;right:12px;bottom:16px;display:none;gap:9px;pointer-events:auto;z-index:50}
.mobileBtns button{width:64px;height:64px;border-radius:50%;padding:0;background:#0c1726cc;border:1px solid #6f91b1aa;box-shadow:0 5px 20px #0008}
.mobileBtns button:first-child{width:72px;height:72px;background:#15304ddd;border-color:#8ed7ffcc}
...
@media (max-width:700px), (pointer:coarse){
 ...
 #aimZone{display:block}
 .menu{width:calc(100vw - 28px);max-width:520px;padding:18px;margin:8px auto}
 .brand{font-size:38px;letter-spacing:5px;white-space:nowrap}
 .sub{font-size:16px}
 .menu .grid{gap:10px}
 .menu button{min-height:62px;font-size:18px}
 .panel{box-sizing:border-box}
 #buy{width:calc(100vw - 28px);max-height:78dvh;overflow:auto;padding:16px}
 #buy button{min-height:58px}
 #joy,.mobileBtns{display:flex}
...
<div id="menu" class="screen">
 <div class="panel menu">
  ...
  <div class="grid">
   <button type="button" onclick="startGame();event.stopPropagation()">▶ PLAY</button>
   <button type="button" onclick="show('loadout');event.stopPropagation()">🔫 LOADOUT</button>
   <button type="button" onclick="show('agents');event.stopPropagation()">⚡ AGENTS</button>
   <button type="button" onclick="show('aim');event.stopPropagation()">🎯 AIM / ESP</button>
   <button type="button" onclick="show('shop');event.stopPropagation()">🛒 SHOP</button>
   <button type="button" onclick="show('profile');event.stopPropagation()">👤 PROFILE</button>
  </div>
  <button type="button" class="full" onclick="show('settings');event.stopPropagation()">⚙ SETTINGS</button>
 </div>
</div>
...
<div id="game" class="hidden">
 ...
 <div id="aimZone"></div><div id="joy"><div id="knob"></div></div>
...
function show(id){
 ...
 if(!target){ console.error('Screen not found:',id); return false; }
...
let joyPointer=null,jx=0,jy=0;
let aimPointer=null,lastAimX=0,lastAimY=0;
...
function joyMove(clientX,clientY){
  ...
  keys.w=jy<-.22; keys.s=jy>.22; keys.a=jx<-.22; keys.d=jx>.22;
}
function joyReset(){
  joyPointer=null;jx=jy=0;
...
joy.addEventListener('pointerdown',e=>{
  ...
  joyPointer=e.pointerId; joy.setPointerCapture?.(e.pointerId);
  joyMove(e.clientX,e.clientY); e.preventDefault();
},{passive:false});
joy.addEventListener('pointermove',e=>{
  if(e.pointerId===joyPointer){joyMove(e.clientX,e.clientY);e.preventDefault();}
},{passive:false});
joy.addEventListener('pointerup',e=>{if(e.pointerId===joyPointer)joyReset()},{passive:false});
joy.addEventListener('pointercancel',joyReset,{passive:false});

aimZone.addEventListener('pointerdown',e=>{
  if(!touchDevice)return;
  aimPointer=e.pointerId; aimZone.setPointerCapture?.(e.pointerId);
  lastAimX=e.clientX; lastAimY=e.clientY; e.preventDefault();
},{passive:false});
aimZone.addEventListener('pointermove',e=>{
  if(e.pointerId===aimPointer){
    const dx=e.clientX-lastAimX;
    const dy=e.clientY-lastAimY;
    player.a += dx*0.005;
    lastAimX=e.clientX; lastAimY=e.clientY;
    e.preventDefault();
  }
},{passive:false});
aimZone.addEventListener('pointerup',e=>{if(e.pointerId===aimPointer)aimPointer=null},{passive:false});
aimZone.addEventListener('pointercancel',e=>{if(e.pointerId===aimPointer)aimPointer=null},{passive:false});

// Prevent iOS page gestures while playing.
document.addEventListener('touchmove',e=>{
  if(!document.getElementById('game').classList.contains('hidden')) e.preventDefault();
},{passive:false});

// Mobile action buttons: pointer events work consistently on iPhone Safari.
const fireBtn=document.querySelector('.mobileBtns button:first-child');
fireBtn.addEventListener('pointerdown',e=>{shoot=true;e.preventDefault();},{passive:false});
fireBtn.addEventListener('pointerup',e=>{shoot=false;e.preventDefault();},{passive:false});
fireBtn.addEventListener('pointercancel',()=>shoot=false,{passive:false});
fireBtn.addEventListener('pointerleave',()=>{if(!fireBtn.hasPointerCapture?.())shoot=false});
...
components.html(GAME, height=920, scrolling=False)
